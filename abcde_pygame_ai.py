...
GAME_RULES = [
    ...
    "Sail Limit",
    "If a ship has no port stay or port entry record in the previous 4 completed rounds, it sinks at the start of the next round. In practice this means a ship that remains enroute with only Return or Continue available for 4 rounds sinks unless it enters port first. Unfinished voyage distance is not charged when sunk.",
    "Fees",
...
NOTEBOOK_COURSES = [
    ...
    ("Tierra_del_Fuego", "Antofagasta", 3),
...
class Ship:
    ...
    last_departure_round: int | None = None
...
def can_take_action(player: PlayerState | None) -> bool:
    return bool(player and player.money >= MIN_ACTION_MONEY)


def country_short_label(country: str | None) -> str:
    labels = {
        "United Kingdom": "UK",
        "United States": "US",
        "Russia": "Russia",
    ...
    return labels.get(country, country or "Unknown")


def combat_result_summary(result: dict[str, int | str | bool] | None) -> str:
    if not result:
        return ""
    winner = str(result.get("winner") or "")
    attacker = str(result.get("attacker") or "")
    defender = str(result.get("defender") or "")
    captured_port = str(result.get("captured_port") or "")
    if captured_port:
        loser = defender if winner == attacker else attacker
        return f"{country_short_label(loser)} lost {display_place_name(captured_port)} to {country_short_label(winner)}."
    if result.get("stolen_trade"):
        return f"{country_short_label(winner)} robbed a merchant: {result['stolen_trade']}."
    lost = str(result.get("lost_entity") or "unit")
    return f"{country_short_label(winner)} won. Lost: {lost}."
...
def make_players() -> list[PlayerState]:
    ...
    for country in EMPIRE_ORDER:
        ...
        if country == "Pirates":
            ships = [Ship(f"pirate {index}", country, capital, "Pirate") for index in range(1, 9)]
        else:
            ships = [
                Ship(f"{country} merchant 1", country, capital, "Merchant"),
                Ship(f"{country} warship 1", country, capital, "Warship"),
                Ship(f"{country} warship 2", country, capital, "Warship"),
                Ship(f"{country} merchant 2", country, capital, "Merchant"),
            ]
        players.append(
            PlayerState(
                country=country,
...
def randomize_game() -> list[PlayerState]:
    ...
    for player in players:
        ...
        ship_count = 8 if player.country == "Pirates" else 4
        for index in range(ship_count):
            kind = "Pirate" if player.country == "Pirates" else ("Merchant" if index in (0, 3) else "Warship")
            name = f"pirate {index + 1}" if player.country == "Pirates" else f"{player.country} {kind.lower()} {index + 1}"
            player.ships.append(
                Ship(
                    name=name,
...
def clamp_non_gold_resources(player: PlayerState) -> None:
    for resource in RESOURCE_ORDER:
        if resource != "gold" and player.resources.get(resource, 0) > 10:
...
def entity_power(
    ...
    if (
        isinstance(entity, MapNode)
        and round_number is not None
        and entity.pirate_rob_power_penalty_until >= round_number
    ):
        penalty += 1
    bonus = 0
...
def ship_route_location_label(ship: Ship) -> str:
    if ship.destination:
        start = ship.last_departure_port or ship.location
        return f"Sea lane: {display_place_name(start)} -> {display_place_name(ship.destination)}"
    return f"Port: {display_place_name(ship.location)}"


def merchant_trade_route_label(ship: Ship) -> str:
    if ship.trade_card:
        return f"Merchant route: {display_place_name(ship.trade_card.start)} -> {display_place_name(ship.trade_card.end)}"
    return "Merchant route: empty merchant"
...
def convert_robbed_merchant_to_pirate(ship: Ship, players: list[PlayerState]) -> str | None:
    ...
    ship.owner = "Pirates"
    ship.kind = "Pirate"
    ship.name = f"pirate {len(pirate_player.ships) + 1}"
    pirate_player.ships.append(ship)
    return stolen_detail or f"{old_kind} converted to pirate"
...
def resolve_attack(
    ...
    rob_location = ""
    rob_trade_route = ""
    if action_kind == "rob" and isinstance(defender_entity, Ship):
        rob_location = ship_route_location_label(defender_entity)
        rob_trade_route = merchant_trade_route_label(defender_entity)
    ...
    while attacker_power == defender_power:
        ...
        defender_entity_power = entity_power(defender_entity, power_penalties, players, round_number)
        if (
            isinstance(defender_entity, Ship)
            and defender_entity.kind == "Merchant"
            and (prompt.attacker.kind == "Pirate" or prompt.attacker.owner == "Pirates")
        ):
            defender_entity_power = 1
        attacker_power = attacker_roll * attacker_entity_power
        defender_power = defender_roll * defender_entity_power
    ...
    if attacker_wins and isinstance(loser, Ship) and loser is prompt.defender:
        if prompt.attacker.owner == "Pirates" or prompt.attacker.kind == "Pirate":
            old_kind = ship_kind_label(loser, players)
            stolen_detail = convert_robbed_merchant_to_pirate(loser, players)
            converted_ship = True
            loser_label = f"{old_kind} converted to Pirates"
        elif action_kind == "rob":
    ...
    return {
        ...
        "rob_location": rob_location,
        "rob_trade_route": rob_trade_route,
        "rob_result": f"Winner: {prompt.attacker.owner if attacker_wins else entity_owner(prompt.defender) or 'Unknown'} | Lost: {loser_label}" if action_kind == "rob" else "",
...
def shortest_course_path(start: str, goal: str) -> list[str]:
    if start == goal:
        return [start]
...
def update_cash_records(players: list[PlayerState]) -> None:
    for player in players:
        ...
        if len(player.cash_history) > 3:
            del player.cash_history[: len(player.cash_history) - 3]
        if len(player.cash_history) == 3:
            player.historic_cash_peak_3_rounds = max(
                player.historic_cash_peak_3_rounds,
                min(player.cash_history),
            )
...
def base_ship_speed(ship: Ship) -> int:
    if ship.kind == "Merchant":
        return 1 if ship.trade_card else 2
...
def port_allows_ship_entry(
    ...
    if ship_has_license(ship, "smuggler", players, player):
        return True
    if ship.owner == "Pirates" and round_number is not None and node.pirate_block_until >= round_number:
        return False
    if ship.kind == "Merchant":
        return True
    if ship.owner == "Japan":
...
def can_ship_enter_port(
    ...
    if destination in plague_ports and ship.location not in plague_ports:
        return False
    return port_allows_ship_entry(node, ship, players, player, round_number)


def port_entry_policy_line(node: MapNode) -> str:
    if node.owner == "Pirates":
        return "Pirate ports: open/free"
    if node.pirate_block_until:
        return f"Pirates rejected through R{node.pirate_block_until}"
...
def complete_trade_if_ready(
    ...
    route_path = shortest_course_path(route_start, ship.location)
    route_detail = "Route ports: " + " -> ".join(display_place_name(port) for port in route_path) if route_path else "Route ports: unknown"
    history_events.append(
        HistoryEvent(
            round_number=round_number,
            kind="trade",
            title=f"{player.country} completed a trade route",
            details=[f"{card.start} -> {card.end}", f"Profit: ${profit}", f"Ship: {ship.name}", route_detail],
        )
    )
...
def move_ship_toward(
    ...
    if not can_take_action(player_by_country(players, ship.owner)):
        return False
    plague_ports = active_plague_ports(active_news or [])
    ...
    if distance is None:
        return False
    departure_port = ship.location
    ship.destination = destination
    ship.course_distance = distance
    ship.progress = 0
    ship.sugar_speed_active = False
    ship.last_port_round = round_number or ship.last_port_round
    ship.last_departure_round = round_number
    ship.last_departure_port = departure_port
...
def draw_enroute_action_popup(
    ...
    pos = ship_screen_position(ship)
    can_operate = can_take_action(player)
    can_speed = can_operate and player.resources.get("sugar", 0) > 0 and not ship.sugar_speed_active
    ...
    pygame.draw.rect(surface, (65, 96, 123) if can_operate else (130, 138, 142), continue_rect, border_radius=5)
    pygame.draw.rect(surface, (116, 86, 151) if can_operate else (130, 138, 142), return_rect, border_radius=5)
...
def draw_enroute_target_popup(
    ...
    can_operate = can_take_action(player_by_country(players, ship.owner))
    can_attack = can_operate and ship_can_attack(ship, players)
    ...
    pygame.draw.rect(surface, (67, 132, 221) if can_operate else (130, 138, 142), join_rect, border_radius=5)
...
def hovered_port_action(
    ...
    node = MAP_NODES.get(destination) if destination else None
    can_force_attack = bool(
        destination
        and node
    ...
    if destination and (
        (MAP_NODES[destination].owner != ship.owner and can_ship_enter_port(ship, destination, active_news, player=player, round_number=current_round))
...
def draw_port_action_popup(
    ...
    peaceful_entry_allowed = same_port or can_ship_enter_port(ship, destination, active_news, player=player, round_number=round_number)
    ...
    button_widths = []
    if can_operate and not same_port and reachable_by_sea and peaceful_entry_allowed:
...
def draw_crown_glyph(surface: pygame.Surface, rect: pygame.Rect) -> None:
    points = [
        (rect.left + 1, rect.bottom - 3),
        (rect.left + 1, rect.top + 5),
        (rect.left + rect.width // 4, rect.top + 1),
        (rect.centerx, rect.top + 6),
        (rect.right - rect.width // 4, rect.top + 1),
        (rect.right - 1, rect.top + 5),
        (rect.right - 1, rect.bottom - 3),
    ]
    pygame.draw.polygon(surface, (224, 180, 57), points)
    pygame.draw.rect(surface, (156, 106, 32), (rect.left + 1, rect.bottom - 5, rect.width - 2, 4), border_radius=1)
    pygame.draw.lines(surface, (92, 64, 22), True, points, 1)
    for jewel_x in (rect.left + rect.width // 4, rect.centerx, rect.right - rect.width // 4):
        pygame.draw.circle(surface, (255, 240, 140), (jewel_x, rect.top + 5), 1)
...
def draw_ports(
    ...
    for node in MAP_NODES.values():
        ...
        if node.kind == "capital":
            ...
            draw_crown_glyph(surface, pygame.Rect(pos[0] - 9, pos[1] - 27, 18, 15))
...
def draw_news_port_highlights(surface: pygame.Surface, active_news: list[NewsEvent]) -> None:
    for port_name in active_monsoon_ports(active_news):
        if port_name in MAP_NODES:
            pygame.draw.circle(surface, (58, 190, 96), node_to_screen(port_name), 27, 4)
    for port_name in active_plague_ports(active_news):
        if port_name in MAP_NODES:
            pygame.draw.circle(surface, (226, 50, 50), node_to_screen(port_name), 31, 4)
...
def draw_player_tabs(
    ...
    def ship_mix(player: PlayerState) -> str:
        counts = {
            "M": sum(1 for ship in player.ships if ship.kind == "Merchant"),
            "W": sum(1 for ship in player.ships if ship.kind == "Warship"),
            "P": sum(1 for ship in player.ships if ship.kind == "Pirate"),
        }
        parts = [f"{label}{amount}" for label, amount in counts.items() if amount]
        return f"S{len(player.ships)} " + "/".join(parts)

    def entry_summary(player: PlayerState) -> str:
        owned_ports = [node for node in MAP_NODES.values() if node.owner == player.country]
        if player.country == "Pirates":
            return "Open/free"
        modes = defaultdict(int)
        for node in owned_ports:
            modes[node.entry_mode] += 1
        if modes.get("reject_selected") or modes.get("allow_only") or modes.get("reject_all"):
            return f"Restrict {modes.get('reject_selected', 0) + modes.get('allow_only', 0) + modes.get('reject_all', 0)}"
        if modes.get("open_all"):
            return f"Open {modes['open_all']}"
        return "Default"
    ...
    for idx, player in enumerate(players):
        ...
        draw_text(surface, font, player.country, (x + 8, y + 5), text_color)
        draw_text(surface, font, ship_mix(player), (x + 8, y + 22), text_color)
        draw_text(surface, font, entry_summary(player), (x + 86, y + 22), text_color)
        x += w + 8
...
def pirate_intel_endpoint_candidates(exclude: str | None = None) -> list[str]:
    return [
        name
        for name, node in MAP_NODES.items()
        if name != exclude and node.kind != "choking"
    ]


def uncertain_port_name(true_port: str, correct_chance: float, *, allow_choking: bool = True) -> str:
    true_node = MAP_NODES.get(true_port)
    if random.random() < correct_chance and (allow_choking or not true_node or true_node.kind != "choking"):
        return true_port
    alternatives = (
        [name for name in MAP_NODES if name != true_port]
        if allow_choking
        else pirate_intel_endpoint_candidates(true_port)
    )
    return random.choice(alternatives) if alternatives else true_port


def generate_pirate_intelligence(
    players: list[PlayerState],
    port_storage_by_port: dict[str, list[StoredGood]],
    ...
    for player in players:
        if player.country == "Pirates":
            continue
        for ship in player.ships:
            if ship.kind != "Merchant" or not ship.trade_card:
                continue
            shown_start = uncertain_port_name(ship.trade_card.start, 1.00, allow_choking=False)
            shown_end = uncertain_port_name(ship.trade_card.end, 0.40, allow_choking=False)
            location = ship_route_location_label(ship)
            lines.append(
                f"{player.country} | Start: {display_place_name(shown_start)} | {location} | End intel: {display_place_name(shown_end)}"
            )
            target_ports.extend([ship.location, shown_end])
            declared_count += 1
    for port_name, goods in port_storage_by_port.items():
        for good in goods:
            if good.owner == "Pirates":
                continue
            shown_start = uncertain_port_name(good.trade_card.start, 1.00, allow_choking=False)
            shown_end = uncertain_port_name(good.trade_card.end, 0.40, allow_choking=False)
            lines.append(
                f"{good.owner} | Start: {display_place_name(shown_start)} | Cargo stored at {display_place_name(port_name)} | End intel: {display_place_name(shown_end)}"
            )
            target_ports.extend([port_name, shown_end])
            declared_count += 1
    if declared_count == 0:
        lines.append("No declared route found.")
    lines.append("UNDECLARED ROUTES")
    undeclared_count = 0
    for player in players:
        if player.country == "Pirates":
            continue
        for card in player.trade_cards:
            if card.status != "undeclared":
                continue
            shown_start = uncertain_port_name(card.start, 0.60, allow_choking=False)
            lines.append(f"{player.country} | Start intel: {display_place_name(shown_start)}")
            target_ports.append(shown_start)
            undeclared_count += 1
    if undeclared_count == 0:
        lines.append("No undeclared route found.")
    return lines, [port for port in target_ports if port in MAP_NODES]


def draw_market_button(surface: pygame.Surface, font: pygame.font.Font) -> pygame.Rect:
    rect = pygame.Rect(MAP_RECT.left, SCREEN_HEIGHT - 48, 84, 34)
    pygame.draw.rect(surface, (67, 132, 221), rect, border_radius=7)
    pygame.draw.rect(surface, (224, 232, 238), rect, 1, border_radius=7)
    draw_text(surface, font, "Market", (rect.x + 14, rect.y + 8), TEXT)
    return rect


def draw_trade_card_button(surface: pygame.Surface, font: pygame.font.Font) -> pygame.Rect:
    rect = pygame.Rect(MAP_RECT.left + 94, SCREEN_HEIGHT - 48, 84, 34)
    pygame.draw.rect(surface, (54, 130, 94), rect, border_radius=7)
    pygame.draw.rect(surface, (224, 232, 238), rect, 1, border_radius=7)
    draw_text(surface, font, "Invest", (rect.x + 16, rect.y + 8), TEXT)
    return rect


def draw_new_treaty_button(surface: pygame.Surface, font: pygame.font.Font) -> pygame.Rect:
    rect = pygame.Rect(MAP_RECT.left + 188, SCREEN_HEIGHT - 48, 118, 34)
    pygame.draw.rect(surface, (126, 93, 55), rect, border_radius=7)
    pygame.draw.rect(surface, (224, 232, 238), rect, 1, border_radius=7)
    draw_text(surface, font, "New Treaty", (rect.x + 10, rect.y + 8), TEXT)
    return rect


def draw_transfer_button(surface: pygame.Surface, font: pygame.font.Font) -> pygame.Rect:
    rect = pygame.Rect(MAP_RECT.left + 316, SCREEN_HEIGHT - 48, 104, 34)
    pygame.draw.rect(surface, (116, 86, 151), rect, border_radius=7)
    pygame.draw.rect(surface, (224, 232, 238), rect, 1, border_radius=7)
    draw_text(surface, font, "Transfer", (rect.x + 14, rect.y + 8), TEXT)
    return rect


def draw_history_button(surface: pygame.Surface, font: pygame.font.Font) -> pygame.Rect:
    rect = pygame.Rect(MAP_RECT.left + 430, SCREEN_HEIGHT - 48, 92, 34)
    pygame.draw.rect(surface, (84, 104, 116), rect, border_radius=7)
    pygame.draw.rect(surface, (224, 232, 238), rect, 1, border_radius=7)
    draw_text(surface, font, "History", (rect.x + 14, rect.y + 8), TEXT)
    return rect


def draw_new_factory_button(surface: pygame.Surface, font: pygame.font.Font) -> pygame.Rect:
    rect = pygame.Rect(MAP_RECT.left + 532, SCREEN_HEIGHT - 48, 104, 34)
    pygame.draw.rect(surface, (52, 121, 82), rect, border_radius=7)
    pygame.draw.rect(surface, (224, 232, 238), rect, 1, border_radius=7)
    draw_text(surface, font, "Arrange", (rect.x + 16, rect.y + 8), TEXT)
    return rect


def draw_new_ship_button(surface: pygame.Surface, font: pygame.font.Font) -> pygame.Rect:
    rect = pygame.Rect(MAP_RECT.left + 646, SCREEN_HEIGHT - 48, 96, 34)
...
def draw_rules_button(surface: pygame.Surface, font: pygame.font.Font) -> pygame.Rect:
    rect = pygame.Rect(MAP_RECT.left + 864, SCREEN_HEIGHT - 48, 78, 34)
    pygame.draw.rect(surface, (90, 98, 58), rect, border_radius=7)
    pygame.draw.rect(surface, (224, 232, 238), rect, 1, border_radius=7)
    draw_text(surface, font, "Rules", (rect.x + 16, rect.y + 8), TEXT)
    return rect


def draw_overview_button(surface: pygame.Surface, font: pygame.font.Font) -> pygame.Rect:
    rect = pygame.Rect(MAP_RECT.left + 752, SCREEN_HEIGHT - 48, 102, 34)
    pygame.draw.rect(surface, (78, 104, 136), rect, border_radius=7)
    pygame.draw.rect(surface, (224, 232, 238), rect, 1, border_radius=7)
    draw_text(surface, font, "Overview", (rect.x + 12, rect.y + 8), TEXT)
    return rect
...
def can_build_factory(player: PlayerState, node: MapNode, action: str, prices: dict[str, dict[str, int]]) -> bool:
    ...
    if player.country == "Pirates" and node.resource == "shipyard":
        return False
    if player.country == "Pirates" and action in ("red", "upgrade"):
        return False
...
def draw_pirate_intel_line(
    ...
    if " | " not in line:
        draw_wrapped_text(surface, font, line, rect, color, 2)
        return
    country, details = line.split(" | ", 1)
    flag_rect = pygame.Rect(rect.x, rect.y + 2, 58, 26)
    pygame.draw.rect(surface, OWNER_COLORS.get(country, OWNER_COLORS[None]), flag_rect, border_radius=5)
    pygame.draw.rect(surface, (70, 88, 101), flag_rect, 1, border_radius=5)
    icon = OWNER_ICONS.get(country, "")
    label = {
        "United Kingdom": "UK",
        "Russia": "RU",
        "China": "CN",
        "Japan": "JP",
        "United States": "US",
    }.get(country, OWNER_LEGEND_LABELS.get(country, country[:3]))
    draw_text(surface, small_font, f"{icon} {label}", (flag_rect.x + 5, flag_rect.y + 6), (22, 38, 52))
    text_rect = pygame.Rect(rect.x + 68, rect.y, rect.width - 68, rect.height)
    draw_wrapped_text(surface, font, details, text_rect, color, 2)


def draw_pirate_intel_page(
    surface: pygame.Surface,
    title_font: pygame.font.Font,
    font: pygame.font.Font,
    small_font: pygame.font.Font,
    ...
    surface.fill((18, 28, 38))
    panel = pygame.Rect(150, 72, SCREEN_WIDTH - 300, SCREEN_HEIGHT - 144)
    pygame.draw.rect(surface, (240, 239, 229), panel, border_radius=8)
    pygame.draw.rect(surface, (70, 88, 101), panel, 2, border_radius=8)
    x = panel.x + 38
    y = panel.y + 28
    draw_text(surface, title_font, "Pirate Intelligence", (x, y), (22, 38, 52))
    next_cost = PIRATE_INTEL_COSTS[clicks_used] if clicks_used < len(PIRATE_INTEL_COSTS) else None
    cost_text = "No checks left" if next_cost is None else f"Next check: ${next_cost}"
    draw_text(surface, font, f"Checks used: {clicks_used}/5   {cost_text}", (x, y + 44), (132, 86, 44))
    close_rect = pygame.Rect(panel.right - 104, panel.y + 24, 70, 32)
    pygame.draw.rect(surface, (78, 91, 103), close_rect, border_radius=6)
    draw_text(surface, small_font, "Close", (close_rect.x + 18, close_rect.y + 8), TEXT)
    y += 92
    for line in intel_lines[:18]:
        color = (132, 86, 44) if line in ("DECLARED ROUTES", "UNDECLARED ROUTES") else (42, 58, 70)
        line_rect = pygame.Rect(x, y, panel.width - 76, 46)
        draw_pirate_intel_line(surface, font, small_font, line, line_rect, color)
        y += 48
        if y > panel.bottom - 50:
            break
    draw_text(surface, small_font, "Esc or Close returns to the map.", (panel.x + 38, panel.bottom - 38), (87, 101, 112))
    return close_rect
...
def draw_market_page(
    ...
    for resource in RESOURCE_ORDER:
        ...
        buy_enabled = can_take_action(player) and player.money >= buy_price
...
def random_trade_card_news_cost() -> int:
    while True:
        cost = round(random.gauss(190, 25))
        if 125 <= cost <= 275:
            return cost
...
def create_economic_news(round_number: int, prices: dict[str, dict[str, int]]) -> NewsEvent:
    if random.choice([True, False]):
        ...
        return NewsEvent(
            ...
            details=[
                f"{RESOURCE_LABELS[resource]} buy/sell prices {direction} by ${abs(delta)}.",
                "Temporary market effect lasts 3 rounds and affects both buy and sell prices.",
            ],
            expires_round=round_number + 2,
            resource=resource,
            price_delta=delta,
        )
    cost = random_trade_card_news_cost()
    return NewsEvent(
        category="economic",
...
def port_ocean_label(node: MapNode) -> str:
    if node.lat >= 62:
        return "Arctic Ocean"
    if -8 <= node.lon <= 42 and node.lat >= 28:
        return "Mediterranean"
    if 20 <= node.lon <= 120 and -45 <= node.lat <= 30:
        return "Indian Ocean"
    if node.lon >= 120 or node.lon <= -105:
        return "Pacific Ocean"
    return "Atlantic Ocean"


def factory_overview_label(node: MapNode) -> str:
    if not node.factory_level:
        return "None"
    owner = OWNER_LEGEND_LABELS.get(node.factory_owner, node.factory_owner or "")
    return f"{node.factory_level} ({owner})" if owner else node.factory_level


def draw_shipyard_glyph(surface: pygame.Surface, rect: pygame.Rect) -> None:
    pygame.draw.rect(surface, (45, 95, 132), rect, border_radius=5)
    pygame.draw.rect(surface, (220, 226, 229), rect, 1, border_radius=5)
    mast_x = rect.centerx
    pygame.draw.line(surface, (235, 241, 238), (mast_x, rect.top + 4), (mast_x, rect.bottom - 5), 2)
    pygame.draw.polygon(
        surface,
        (235, 241, 238),
        [(mast_x + 2, rect.top + 5), (mast_x + 2, rect.centery + 3), (rect.right - 5, rect.centery + 3)],
    )
    pygame.draw.polygon(
        surface,
        (18, 35, 50),
        [(rect.left + 5, rect.bottom - 8), (rect.right - 5, rect.bottom - 8), (rect.right - 9, rect.bottom - 3), (rect.left + 9, rect.bottom - 3)],
    )


def draw_warehouse_glyph(surface: pygame.Surface, rect: pygame.Rect) -> None:
    pygame.draw.rect(surface, (150, 101, 55), rect, border_radius=4)
    pygame.draw.rect(surface, (82, 52, 27), rect, 1, border_radius=4)
    pygame.draw.line(surface, (218, 170, 100), rect.midleft, rect.midright, 2)
    pygame.draw.line(surface, (96, 60, 30), (rect.centerx, rect.top + 2), (rect.centerx, rect.bottom - 2), 1)


def draw_no_resource_glyph(surface: pygame.Surface, rect: pygame.Rect) -> None:
    pygame.draw.rect(surface, (235, 224, 218), rect, border_radius=5)
    pygame.draw.rect(surface, (157, 63, 58), rect, 1, border_radius=5)
    pygame.draw.line(surface, (157, 63, 58), (rect.left + 6, rect.top + 5), (rect.right - 6, rect.bottom - 5), 3)
    pygame.draw.line(surface, (157, 63, 58), (rect.left + 6, rect.bottom - 5), (rect.right - 6, rect.top + 5), 3)


def draw_anchor_glyph(surface: pygame.Surface, rect: pygame.Rect) -> None:
    pygame.draw.rect(surface, (70, 153, 232), rect, border_radius=5)
    pygame.draw.rect(surface, (220, 226, 229), rect, 1, border_radius=5)
    color = (245, 250, 252)
    pygame.draw.circle(surface, color, (rect.centerx, rect.top + 5), 3, 2)
    pygame.draw.line(surface, color, (rect.centerx, rect.top + 8), (rect.centerx, rect.bottom - 5), 2)
    pygame.draw.line(surface, color, (rect.left + 6, rect.top + 11), (rect.right - 6, rect.top + 11), 2)
    pygame.draw.arc(surface, color, rect.inflate(-6, -4).move(0, 1), math.radians(25), math.radians(155), 2)
    pygame.draw.line(surface, color, (rect.left + 5, rect.bottom - 7), (rect.left + 8, rect.bottom - 4), 2)
    pygame.draw.line(surface, color, (rect.right - 5, rect.bottom - 7), (rect.right - 8, rect.bottom - 4), 2)


def draw_castle_glyph(surface: pygame.Surface, rect: pygame.Rect) -> None:
    pygame.draw.rect(surface, (230, 79, 79), rect, border_radius=5)
    pygame.draw.rect(surface, (220, 226, 229), rect, 1, border_radius=5)
    stone = (250, 232, 214)
    pygame.draw.rect(surface, stone, (rect.left + 4, rect.top + 8, rect.width - 8, rect.height - 7), border_radius=1)
    for tower_x in (rect.left + 4, rect.centerx - 3, rect.right - 10):
        pygame.draw.rect(surface, stone, (tower_x, rect.top + 4, 6, 8))
        pygame.draw.rect(surface, (230, 79, 79), (tower_x + 1, rect.top + 4, 2, 3))
    pygame.draw.rect(surface, (88, 52, 45), (rect.centerx - 3, rect.bottom - 7, 6, 7), border_radius=2)


def draw_overview_resource_cell(
    surface: pygame.Surface,
    small_font: pygame.font.Font,
    node: MapNode,
    pos: tuple[int, int],
    ...
    x, y = pos
    if node.kind == "capital" or node.resource == "capital":
        draw_crown_glyph(surface, pygame.Rect(x, y + 1, 22, 18))
        draw_text(surface, small_font, "capital"[: max(4, (width - 28) // 7)], (x + 28, y + 2), (22, 38, 52))
        return
    if node.kind == "choking":
        draw_no_resource_glyph(surface, pygame.Rect(x, y + 1, 24, 18))
        draw_text(surface, small_font, "none", (x + 30, y + 2), (22, 38, 52))
        return
    resource = node.resource or "warehouse"
    text_x = x
    if resource in RESOURCE_ORDER:
        draw_resource_glyph(surface, resource, pygame.Rect(x, y + 1, 24, 18), small_font, show_label=False)
        text_x = x + 30
    elif resource == "shipyard":
        draw_shipyard_glyph(surface, pygame.Rect(x, y + 1, 24, 18))
        text_x = x + 30
    elif resource == "warehouse":
        draw_warehouse_glyph(surface, pygame.Rect(x, y + 1, 24, 18))
        text_x = x + 30
    label = RESOURCE_LABELS.get(resource, resource)
    draw_text(surface, small_font, label[: max(4, (width - (text_x - x)) // 7)], (text_x, y + 2), (22, 38, 52))


def draw_overview_type_cell(
    surface: pygame.Surface,
    small_font: pygame.font.Font,
    node: MapNode,
    pos: tuple[int, int],
    ...
    if node.kind == "capital":
        ...
        return
    if node.kind == "port":
        draw_anchor_glyph(surface, pygame.Rect(x, y + 1, 22, 18))
        draw_text(surface, small_font, "port"[: max(4, (width - 28) // 7)], (x + 28, y + 2), (22, 38, 52))
        return
    if node.kind == "choking":
        draw_castle_glyph(surface, pygame.Rect(x, y + 1, 22, 18))
        draw_text(surface, small_font, "choking"[: max(4, (width - 28) // 7)], (x + 28, y + 2), (22, 38, 52))
        return
    label = node_type_label(node)
    draw_text(surface, small_font, label[: max(4, width // 7)], (x, y + 2), (22, 38, 52))


def draw_factory_house(surface: pygame.Surface, rect: pygame.Rect, level: str | None) -> None:
    if not level:
        draw_text(surface, pygame.font.SysFont("arial", 16), "-", (rect.x + 8, rect.y + 4), (87, 101, 112))
        return
    color = (54, 130, 94) if level == "green" else (157, 63, 58)
    roof_color = (32, 66, 52) if level == "green" else (92, 42, 40)
    roof = [
        (rect.left + 1, rect.top + 11),
        (rect.centerx, rect.top + 2),
        (rect.right - 1, rect.top + 11),
    ]
    body = pygame.Rect(rect.left + 4, rect.top + 10, rect.width - 8, rect.height - 11)
    pygame.draw.polygon(surface, roof_color, roof)
    pygame.draw.rect(surface, color, body, border_radius=2)
    pygame.draw.rect(surface, (239, 244, 239), (rect.centerx - 3, rect.top + 15, 6, rect.height - 16), border_radius=1)
    pygame.draw.lines(surface, (55, 67, 72), True, [roof[0], roof[1], roof[2], (body.right, body.top), (body.right, body.bottom), (body.left, body.bottom), (body.left, body.top)], 1)


def ships_docked_at_port(players: list[PlayerState], port_name: str) -> list[Ship]:
    ships = []
    for player in players:
        for ship in player.ships:
            if not ship_is_enroute(ship) and ship.location == port_name:
                ships.append(ship)
    return ships


def draw_overview_ships_cell(
    surface: pygame.Surface,
    small_font: pygame.font.Font,
    players: list[PlayerState],
    node: MapNode,
    ...
    docked = ships_docked_at_port(players, node.name)
    if not docked:
        draw_text(surface, small_font, "-", pos, (87, 101, 112))
        return
    x, y = pos
    line_y = y
    for ship in docked[:2]:
        draw_owner_marker(surface, ship.owner, (x + 10, line_y + 9), small_font, size=18)
        label = f"{ship.name}/{ship_kind_label(ship, players)}"
        draw_text(surface, small_font, label[: max(8, (width - 28) // 7)], (x + 24, line_y), (22, 38, 52))
        line_y += 18
    if len(docked) > 2:
        draw_text(surface, small_font, f"+{len(docked) - 2}", (x + width - 24, y + 18), (87, 101, 112))
...
def draw_overview_page(
    ...
    x = panel.x + 30
    y = panel.y + 22
    draw_text(surface, title_font, "Port Overview", (x, y), (22, 38, 52))
    close_rect = pygame.Rect(panel.right - 104, panel.y + 24, 70, 32)
    pygame.draw.rect(surface, (78, 91, 103), close_rect, border_radius=6)
    draw_text(surface, small_font, "Close", (close_rect.x + 18, close_rect.y + 8), TEXT)
    ...
    columns = [
        ...
        ("Owner", x + 620, 58),
        ("Ships", x + 688, 300),
        ("Entry", x + 1000, panel.right - x - 1030),
    ]
    for label, col_x, _ in columns:
        draw_text(surface, small_font, label, (col_x, header_y), (42, 58, 70))

    content_top = header_y + 28
    content_bottom = panel.bottom - 52
    row_height = 46
    ports = sorted(MAP_NODES.values(), key=lambda node: (port_ocean_label(node), node.name))
    max_scroll = max(0, len(ports) * row_height - (content_bottom - content_top))
    scroll_offset = max(0, min(max_scroll, scroll_offset))

    previous_clip = surface.get_clip()
    surface.set_clip(pygame.Rect(panel.x + 20, content_top, panel.width - 40, content_bottom - content_top))
    row_y = content_top - scroll_offset
    for idx, node in enumerate(ports):
        row = pygame.Rect(x - 10, row_y - 5, panel.width - 60, row_height - 3)
        if row.bottom >= content_top and row.top <= content_bottom:
            pygame.draw.rect(surface, (255, 252, 235) if idx % 2 == 0 else (225, 231, 227), row, border_radius=5)
            values = [
                display_place_name(node.name),
                port_ocean_label(node),
                port_entry_policy_line(node),
            ]
            for value, (_, col_x, col_w) in zip(values, columns[:2] + columns[7:]):
                draw_text(surface, small_font, value[: max(6, col_w // 7)], (col_x, row_y + 4), (22, 38, 52))
            draw_overview_type_cell(surface, small_font, node, (columns[2][1], row_y + 3), columns[2][2])
            draw_overview_resource_cell(surface, small_font, node, (columns[3][1], row_y + 3), columns[3][2])
            draw_factory_house(surface, pygame.Rect(columns[4][1], row_y + 1, 28, 24), node.factory_level)
            draw_owner_marker(surface, node.owner, (columns[5][1] + 16, row_y + 12), small_font, size=24)
            draw_overview_ships_cell(surface, small_font, players, node, (columns[6][1], row_y + 4), columns[6][2])
        row_y += row_height
    surface.set_clip(previous_clip)

    if max_scroll:
        draw_text(surface, small_font, "Use mouse wheel to scroll ports.", (panel.right - 236, panel.bottom - 38), (87, 101, 112))
    draw_text(surface, small_font, "Esc or Close returns to the map.", (panel.x + 30, panel.bottom - 38), (87, 101, 112))
    return close_rect, max_scroll
...
def adjust_standard_resource_prices(
    prices: dict[str, dict[str, int]],
    delta: int,
) -> bool:
    changed = False
...
def apply_war_price_increase(
    ...
    standard_key = window
    gold_key = -window - 1
    standard_already = price_increase_by_window.get(standard_key, 0)
    gold_already = price_increase_by_window.get(gold_key, 0)
    standard_increase = min(10, max(0, 30 - standard_already))
    gold_increase = min(20, max(0, 50 - gold_already))
    if standard_increase:
        adjust_standard_resource_prices(prices, standard_increase)
        price_increase_by_window[standard_key] = standard_already + standard_increase
    if gold_increase:
        apply_resource_news_delta(prices, "gold", gold_increase)
        price_increase_by_window[gold_key] = gold_already + gold_increase
    return standard_increase, gold_increase
...
def apply_resource_price_decay(
    ...
    if round_number - last_price_drop_round < 1:
        return 0, 0, last_price_drop_round
    standard_changed = adjust_standard_resource_prices(prices, -5)
    old_price = prices["gold"]["buy"]
    apply_resource_news_delta(prices, "gold", -15)
    gold_decrease = old_price - prices["gold"]["buy"]
    return (5 if standard_changed else 0), gold_decrease, round_number
...
def append_combat_history(
    ...
    if result.get("action_kind") == "rob":
        if result.get("rob_location"):
            details.append(str(result["rob_location"]))
        if result.get("rob_trade_route"):
            details.append(str(result["rob_trade_route"]))
        if result.get("rob_result"):
            details.append(f"Result: {result['rob_result']}")
    ...
    if not is_war:
        return
    standard_increase, gold_increase = price_increase
    price_detail = (
        "No price increase; 5-round caps reached."
        if not standard_increase and not gold_increase
        else f"Standard resources +${standard_increase}; GOLD +${gold_increase}."
    )
    history_events.append(
        HistoryEvent(
            round_number=round_number,
            ...
            details=[price_detail, *price_snapshot_lines(prices)[:3]],
        )
    )
...
def evaluate_players(
    ...
    active_prices = prices or RESOURCE_PRICES
    ...
    score_by_country = {player.country: 0 for player in players}
    detail_by_country: dict[str, list[str]] = {player.country: [] for player in players}
    labels = {
        "money": "cash",
        "ports": "ports",
        "factories": "factories",
    ...
    for key, label in labels.items():
        ...
        for idx, player in enumerate(ranked):
            ...
            detail_by_country[player.country].append(f"{label} {points}")
    rows = sorted(players, key=lambda player: (score_by_country[player.country], player.country), reverse=True)
    return [
        (
...
def port_snapshot_lines(players: list[PlayerState]) -> list[str]:
    lines = []
    for node in sorted(MAP_NODES.values(), key=lambda item: item.name):
        docked = ships_docked_at_port(players, node.name)
        ships = ", ".join(f"{ship.owner} {ship.kind} {ship.name}" for ship in docked) if docked else "none"
        factory = (
            f"{node.factory_level} factory ({node.factory_owner})"
            if node.factory_level
            else "none"
        )
        lines.append(
            f"{display_place_name(node.name)} | resource: {node.resource or 'none'} | factory: {factory} | owner: {node.owner or 'none'} | ships: {ships}"
        )
    return lines


def export_game_history(history_events: list[HistoryEvent], evaluation_lines: list[str], players: list[PlayerState]) -> Path:
    ...
    lines.extend(["PORT SNAPSHOT", *port_snapshot_lines(players), ""])
    lines.extend(["PLAYER TRANSACTIONS", ""])
    for player in players:
        lines.append(player.country)
        lines.extend(player.transactions or ["  none"])
        lines.append("")
...
def transfer_is_valid(
    ...
    for resource in RESOURCE_ORDER:
        ...
        if to_resources.get(resource, 0) > to_player.resources.get(resource, 0):
            return False
        if resource != "gold":
            from_final = from_player.resources.get(resource, 0) - from_resources.get(resource, 0) + to_resources.get(resource, 0)
            to_final = to_player.resources.get(resource, 0) + from_resources.get(resource, 0) - to_resources.get(resource, 0)
            if from_final > 10 or to_final > 10:
                return False
    if not all(any(id(ship) == ship_id for ship in from_player.ships) for ship_id in from_ships):
        return False
    if not all(any(id(ship) == ship_id for ship in to_player.ships) for ship_id in to_ships):
        return False
...
def apply_transfer(
    ...
    if (
        ...
        or from_ships
        or to_ships
        or any(from_resources.values())
        or any(to_resources.values())
    ) and (not can_take_action(from_player) or not can_take_action(to_player)):
        return False
    ...
    moved_from_ships = [ship for ship in from_player.ships if id(ship) in from_ships]
    moved_to_ships = [ship for ship in to_player.ships if id(ship) in to_ships]
    for ship in moved_from_ships:
        from_player.ships.remove(ship)
        ship.owner = to_country
        to_player.ships.append(ship)
        if round_number is not None:
            record_transaction(from_player, round_number, 0, f"ship transferred to {to_country}: {ship_transfer_label(ship)}")
            record_transaction(to_player, round_number, 0, f"ship received from {from_country}: {ship_transfer_label(ship)}")
    for ship in moved_to_ships:
        to_player.ships.remove(ship)
        ship.owner = from_country
        from_player.ships.append(ship)
        if round_number is not None:
            record_transaction(to_player, round_number, 0, f"ship transferred to {from_country}: {ship_transfer_label(ship)}")
            record_transaction(from_player, round_number, 0, f"ship received from {to_country}: {ship_transfer_label(ship)}")
    for resource in RESOURCE_ORDER:
    ...
    clamp_non_gold_resources(from_player)
    clamp_non_gold_resources(to_player)
    refresh_player_ports(players)
    return True
...
def transfer_summary_lines(
    ...
    return [
        f"{from_country} -> {to_country}: ${transfer_amount(from_amount)}, {len(from_ports)} port(s), {len(from_ships)} ship(s), {from_resource_count} resource(s)",
        f"{to_country} -> {from_country}: ${transfer_amount(to_amount)}, {len(to_ports)} port(s), {len(to_ships)} ship(s), {to_resource_count} resource(s)",
    ]


def ship_transfer_label(ship: Ship) -> str:
    if ship.destination:
        location = f"{display_place_name(ship.location)} -> {display_place_name(ship.destination)} {ship.progress}/{ship.course_distance}"
    else:
        location = display_place_name(ship.location)
    cargo = " loaded" if ship.trade_card else ""
    return f"{ship.kind}{cargo} at {location}"
...
def draw_transfer_page(
    ...
    y += 58
    left = pygame.Rect(x, y, (panel.width - 92) // 2, 500)
    ...
    from_ship_rects: list[tuple[int, pygame.Rect]] = []
    to_ship_rects: list[tuple[int, pygame.Rect]] = []
    resource_rects: dict[str, pygame.Rect] = {}
    transfer_max_scrolls = {"from": 0, "to": 0}
    transfer_scroll_areas: dict[str, pygame.Rect] = {}
    for side, rect, payer, receiver, amount, selected_ports, selected_ships, selected_resources, port_rects, ship_rects, scroll in [
        ("from", left, from_country, to_country, from_amount, from_ports, from_ships, from_resources, from_port_rects, from_ship_rects, from_scroll),
        ("to", right, to_country, from_country, to_amount, to_ports, to_ships, to_resources, to_port_rects, to_ship_rects, to_scroll),
    ]:
        ...
        list_area = pygame.Rect(rect.x + 10, rect.y + 198, rect.width - 20, rect.height - 210)
        transfer_scroll_areas[side] = list_area
        pygame.draw.rect(surface, (235, 239, 236), list_area, border_radius=6)
        pygame.draw.rect(surface, (172, 184, 188), list_area, 1, border_radius=6)

        ports = sorted(player.ports, key=lambda port: port.name) if player else []
        ships = sorted(player.ships, key=lambda ship: (ship.kind, ship.location, ship.name)) if player else []
        estimated_content_height = 8 + 24 + len(ports) * 27 + 12 + 24 + len(ships) * 27 + 8
        estimated_max_scroll = max(0, estimated_content_height - list_area.height)
        scroll = max(0, min(estimated_max_scroll, scroll))
        previous_clip = surface.get_clip()
        surface.set_clip(list_area)
        content_x = rect.x + 14
        content_y = list_area.y + 8 - scroll
        draw_text(surface, small_font, "Ports", (content_x, content_y), (58, 72, 82))
        port_y = content_y + 24
        for port in ports:
            port_rect = pygame.Rect(content_x, port_y, rect.width - 28, 24)
            if port_rect.colliderect(list_area):
                port_rects.append((port.name, port_rect.clip(list_area)))
        ...
        ship_header_y = port_y + 12
        draw_text(surface, small_font, "Ships", (content_x, ship_header_y), (58, 72, 82))
        ship_y = ship_header_y + 24
        for ship in ships:
            ship_rect = pygame.Rect(content_x, ship_y, rect.width - 28, 24)
            if ship_rect.colliderect(list_area):
                ship_rects.append((id(ship), ship_rect.clip(list_area)))
            checked = id(ship) in selected_ships
            pygame.draw.rect(surface, (255, 252, 235), ship_rect, border_radius=4)
            pygame.draw.rect(surface, (42, 130, 79) if checked else (132, 142, 149), ship_rect, 2 if checked else 1, border_radius=4)
            label = f"{'[x]' if checked else '[ ]'} {ship_transfer_label(ship)}"
            draw_text(surface, small_font, label[:54], (ship_rect.x + 8, ship_rect.y + 4), (22, 38, 52))
            ship_y += 27
        content_height = ship_y - (list_area.y + 8 - scroll)
        max_scroll = max(0, content_height - list_area.height + 8)
        transfer_max_scrolls[side] = max_scroll
        surface.set_clip(previous_clip)
        if max_scroll:
            bar_h = max(28, int(list_area.height * list_area.height / (content_height + 1)))
            bar_y = list_area.y + int((list_area.height - bar_h) * (scroll / max_scroll))
            pygame.draw.rect(surface, (122, 138, 146), (list_area.right - 8, bar_y, 5, bar_h), border_radius=3)
    ...
    return (
        ...
        amount_rects,
        resource_rects,
        transfer_max_scrolls,
        transfer_scroll_areas,
    )
...
def draw_attack_prompt(
    ...
    attacker_player = player_by_country(players, prompt.attacker.owner)
    attack_enabled = (result is None or can_continue) and can_take_action(attacker_player)
    oil_enabled = bool(result is None and can_take_action(attacker_player) and attacker_player and attacker_player.resources.get("oil", 0) > 0)
...
def draw_combat_result_banner(surface: pygame.Surface, font: pygame.font.Font, text: str) -> None:
    if not text:
        return
    text_surface = font.render(text, True, (252, 247, 232))
    width = min(MAP_RECT.width - 80, text_surface.get_width() + 44)
    rect = pygame.Rect(MAP_RECT.left + (MAP_RECT.width - width) // 2, MAP_RECT.top - 30, width, 28)
    pygame.draw.rect(surface, (112, 42, 45), rect, border_radius=7)
    pygame.draw.rect(surface, (245, 203, 126), rect, 1, border_radius=7)
...
def draw_identity_page(
    ...
    font: pygame.font.Font,
    small_font: pygame.font.Font,
    card_countries: list[str],
    selected_card_idx: int | None,
    mouse_pos: tuple[int, int],
    ...
    draw_text(
        ...
        (SCREEN_WIDTH // 2 - 410, 78),
        (238, 242, 244),
    )
    draw_text(
        surface,
        font,
        "Choose one identity card.",
        (SCREEN_WIDTH // 2 - 92, 136),
        TEXT_MUTED,
    )
    ...
    for idx, country in enumerate(card_countries):
        ...
        if selected:
            draw_owner_marker(surface, country, (rect.centerx, rect.y + 42), font, size=42)
            draw_text(surface, title_font, country, (rect.x + 22, rect.y + 72), (22, 38, 52))
            draw_text(surface, small_font, f"Capital: {display_place_name(CAPITALS[country])}", (rect.x + 22, rect.y + 112), (42, 58, 70))
            draw_text(surface, small_font, "Skill", (rect.x + 22, rect.y + 144), (42, 58, 70))
            draw_wrapped_text(
                surface,
                small_font,
                SKILLS[country],
                pygame.Rect(rect.x + 22, rect.y + 166, rect.width - 44, 56),
                (58, 72, 82),
                3,
            )
            draw_text(surface, small_font, "You are this role.", (rect.x + 22, rect.bottom - 30), (42, 105, 72))
        else:
            pygame.draw.rect(surface, (34, 49, 63), rect.inflate(-36, -36), border_radius=7)
            pygame.draw.rect(surface, (188, 197, 202), rect.inflate(-36, -36), 1, border_radius=7)
            draw_text(surface, title_font, "IDENTITY", (rect.centerx - 62, rect.centery - 12), (238, 242, 244))

    start_rect = pygame.Rect(SCREEN_WIDTH // 2 - 86, SCREEN_HEIGHT - 82, 172, 42)
    if selected_card_idx is not None:
        pygame.draw.rect(surface, (54, 130, 94), start_rect, border_radius=7)
        pygame.draw.rect(surface, (224, 232, 238), start_rect, 1, border_radius=7)
        draw_text(surface, font, "Start Game", (start_rect.x + 42, start_rect.y + 11), TEXT)
...
def main() -> int:
    ...
    overview_button_rect = pygame.Rect(0, 0, 0, 0)
    ...
    rules_max_scroll = 0
    overview_open = False
    overview_close_rect = pygame.Rect(0, 0, 0, 0)
    overview_scroll = 0
    overview_max_scroll = 0
    ...
    transfer_from_ship_rects: list[tuple[int, pygame.Rect]] = []
    transfer_to_ship_rects: list[tuple[int, pygame.Rect]] = []
    transfer_amount_rects: dict[str, pygame.Rect] = {}
    transfer_resource_rects: dict[str, pygame.Rect] = {}
    transfer_scroll_areas: dict[str, pygame.Rect] = {}
    transfer_max_scrolls = {"from": 0, "to": 0}
    transfer_from_scroll = 0
    transfer_to_scroll = 0
    ...
    transfer_from_ships: set[int] = set()
    transfer_to_ships: set[int] = set()
    ...
    def pending_warship_count(player: PlayerState) -> int:
        return sum(1 for build in pending_ship_builds if build.owner == player.country and build.kind == "Warship")

    def player_warship_pipeline_count(player: PlayerState) -> int:
        return player_warship_count(player) + pending_warship_count(player)
    ...
    def apply_pirate_inactivity_port_loss() -> None:
        ...
        if round_number - pirate_last_rob_round < 5:
            return
        pirate_ports = [
            node
            for node in MAP_NODES.values()
            if node.owner == "Pirates" and node.kind != "capital"
        ]
        if not pirate_ports:
            pirate_last_rob_round = round_number
            return
        target_countries = [player for player in players if player.country != "Pirates"]
        if not target_countries:
            return
        fewest_ports = min(len(player.ports) for player in target_countries)
        receiver = random.choice([player for player in target_countries if len(player.ports) == fewest_ports])
        lost_port = random.choice(pirate_ports)
        lost_port.owner = receiver.country
        if lost_port.factory_level:
        ...
        history_events.append(
            HistoryEvent(
                round_number,
                "war",
                "Pirates lost a port from inactivity",
                [
                    "No merchant robbed within 5 rounds.",
                    f"Port: {display_place_name(lost_port.name)}",
                    f"New owner: {receiver.country}",
                ],
            )
        )

    def apply_pirate_rob_port_restrictions(merchant_owner: str, success: bool) -> None:
        affected_ports = [
            node
            for node in MAP_NODES.values()
            if node.owner == merchant_owner and node.kind != "capital"
        ]
        if not affected_ports:
            return
        for node in affected_ports:
            node.pirate_block_until = max(node.pirate_block_until, round_number + 2)
            if success:
                node.pirate_rob_power_penalty_until = max(node.pirate_rob_power_penalty_until, round_number + 2)
        details = [f"{len(affected_ports)} {merchant_owner} ports reject Pirates through R{round_number + 2}."]
        if success:
            details.append(f"Port power -1 through R{round_number + 2}.")
        history_events.append(
            HistoryEvent(
                round_number,
                "war",
                "Pirate rob triggered port security",
                details,
            )
        )
    ...
    def ai_try_queue_ship(agent: AIAgentState, player: PlayerState, force: bool = False) -> bool:
        ...
        needs_warship = player.country != "Pirates" and player_warship_pipeline_count(player) < 2
    ...
    def ai_try_declare_trade(player: PlayerState) -> bool:
        for ship in player.ships:
            ...
            if declare_trade_for_ship(player, ship, ship.location):
                card = ship.trade_card
                if card:
                    if not card.insurance_active and player.money >= trade_insurance_premium(player, card, players, history_events, round_number, late=True):
                        buy_trade_insurance(player, card, players, history_events, round_number, pirate_premium_pool, late=True)
                    history_events.append(
                        HistoryEvent(
                            round_number,
                            "invest",
    ...
    def ai_try_invest(agent: AIAgentState, player: PlayerState, force: bool = False) -> bool:
        ...
        if player.money > 250 and random.random() < 0.40:
            buy_trade_insurance(player, chosen, players, history_events, round_number, pirate_premium_pool)
        history_events.append(
            HistoryEvent(
                round_number,
                "invest",
    ...
    def run_country_agent_decisions(agent: AIAgentState, player: PlayerState) -> None:
        ...
        if player.country == "Russia" and still_cash_poor:
            ...
            if player_warship_pipeline_count(player) < 1:
                ai_try_queue_ship(agent, player, force=True)
            return
        if player.country != "Pirates" and player_warship_pipeline_count(player) < 1:
            if ai_try_queue_ship(agent, player, force=True):
                history_events.append(
                    HistoryEvent(
                        round_number,
                        "ai",
                        f"{player.country} AI prioritized emergency warship build",
                        ["Warship count below 1.", "Queued a replacement before other decisions."],
                    )
                )
                return
    ...
    def finish_game() -> None:
        ...
        game_over_export_path = export_game_history(history_events, game_over_evaluation, players)
        game_over = True
    ...
    def advance_round() -> None:
        ...
        for node in MAP_NODES.values():
            if node.pirate_block_until and node.pirate_block_until < round_number:
        ...
        standard_decrease, gold_decrease, last_price_drop_round = apply_resource_price_decay(
            resource_prices,
            round_number,
            last_price_drop_round,
        )
        if standard_decrease or gold_decrease:
            history_events.append(
                HistoryEvent(
                    round_number=round_number,
                    kind="price",
                    ...
                    details=[
                        f"Standard resources -${standard_decrease}.",
                        f"GOLD price -${gold_decrease}.",
                        *price_snapshot_lines(resource_prices)[:3],
                    ],
                )
            )
    ...
    def resolve_pending_attack_once() -> None:
        ...
        pirate_robbed_merchant_owner = (
            pending_attack.defender.owner
            if pending_attack.attacker.owner == "Pirates"
            and isinstance(pending_attack.defender, Ship)
            and pending_attack.defender.kind == "Merchant"
            else None
        )
        if (
            pending_attack.attacker.owner == "Pirates"
            and isinstance(pending_attack.defender, MapNode)
            and round_number <= pirate_port_raid_bonus_until
        ):
            current_penalty = battle_power_penalties.get(id(pending_attack.defender), 0)
            max_penalty = max(0, base_entity_power(pending_attack.defender) - 1)
            battle_power_penalties[id(pending_attack.defender)] = min(max_penalty, max(current_penalty, 1))
        attack_result = resolve_attack(pending_attack, players, battle_power_penalties, round_number)
        if pirate_robbed_merchant_owner:
            apply_pirate_rob_port_restrictions(
                pirate_robbed_merchant_owner,
        ...
        if ship_is_enroute(ship) and round_number - ship.last_port_round == 4:
            history_events.append(
                HistoryEvent(
                    round_number=round_number,
                    kind="ship",
                    title=f"{ship.owner} ship nearing sail limit",
                    details=[
                        f"{ship.name}: {action_label}",
                        "No port record in the last 4 completed rounds.",
                        f"Will sink in R{round_number + 1} unless it enters port first.",
                    ],
                )
            )
                    ...
                    history_events.append(
                        HistoryEvent(
                            ...
                            details=[
                                ...
                                (
                                    f"Last appear: R{ship.last_departure_round}, {display_place_name(ship.last_departure_port)}"
                                    if ship.last_departure_round is not None and ship.last_departure_port
                                    else f"Last appear: R{ship.last_port_round}, {display_place_name(ship.location)}"
                                ),
                                "Previous 4 rounds had no port stay or port entry.",
    ...
    def auto_advance_enroute_ships_for_active_player() -> bool:
        ...
        for ship in list(active_player().ships):
            ...
            if not ship_is_enroute(ship) and not pending_attack:
                ...
                record_sail_limit_action(ship, "continue")
            if pending_attack:
    ...
    def restart_game() -> None:
        ...
        nonlocal transfer_from_resources, transfer_to_resources, transfer_from_scroll, transfer_to_scroll
        nonlocal history_events, resource_prices, price_increase_by_window, battle_power_penalties, last_price_drop_round
        nonlocal transaction_scroll, transaction_max_scroll, side_panel_scroll, side_panel_max_scroll
        ...
        transfer_from_ships = set()
        transfer_to_ships = set()
        transfer_from_scroll = 0
        transfer_to_scroll = 0
        transfer_from_resources = {resource: 0 for resource in RESOURCE_ORDER}
        transfer_to_resources = {resource: 0 for resource in RESOURCE_ORDER}
    ...
    def open_trade_card_choices() -> None:
        ...
        if player.country == "Pirates":
            if pirate_intel_clicks >= len(PIRATE_INTEL_COSTS) or not can_take_action(player):
                return
            cost = PIRATE_INTEL_COSTS[pirate_intel_clicks]
            if player.money < cost:
                return
            ...
            history_events.append(
                HistoryEvent(
                    round_number,
                    "invest",
                    "Pirates bought intelligence",
                    [f"Cost: ${cost}", f"Check: {pirate_intel_clicks}/5"],
                )
            )
            pirate_intel_open = True
            market_open = False
            trade_card_open = False
            history_open = False
            build_open = False
    ...
    def open_transfer_page() -> None:
        nonlocal transfer_open, market_open, trade_card_open, treaty_open, history_open, build_open, new_ship_open, rules_open, overview_open
        nonlocal transfer_from_country, transfer_to_country, transfer_from_amount, transfer_to_amount
        nonlocal transfer_from_ports, transfer_to_ports, transfer_from_ships, transfer_to_ships
        nonlocal transfer_from_resources, transfer_to_resources, transfer_active_field
        nonlocal transfer_from_scroll, transfer_to_scroll
        transfer_open = True
        market_open = False
        trade_card_open = False
        treaty_open = False
    ...
    def open_build_page() -> None:
        nonlocal build_open, market_open, trade_card_open, treaty_open, transfer_open, history_open, new_ship_open, rules_open, overview_open, arrange_scroll
        build_open = True
        arrange_scroll = 0
        market_open = False
        trade_card_open = False
    ...
    def open_new_ship_page() -> None:
        nonlocal new_ship_open, build_open, market_open, trade_card_open, treaty_open, transfer_open, history_open, rules_open, overview_open
        new_ship_open = True
        build_open = False
        market_open = False
        trade_card_open = False
    ...
    def open_rules_page() -> None:
        nonlocal rules_open, rules_scroll, market_open, trade_card_open, treaty_open, transfer_open, history_open, build_open, new_ship_open, overview_open
        rules_open = True
        rules_scroll = 0
        market_open = False
        trade_card_open = False
    ...
    def open_overview_page() -> None:
        nonlocal overview_open, overview_scroll, market_open, trade_card_open, treaty_open, transfer_open, history_open, build_open, new_ship_open, rules_open
        overview_open = True
        overview_scroll = 0
        market_open = False
        trade_card_open = False
    ...
    def move_selected_ship_to(destination: str, attack_requested: bool) -> None:
        ...
        if not current:
            return
        if not attack_requested and not can_ship_enter_port(current, destination, active_news, players, round_number=round_number):
    ...
    def speed_selected_enroute_ship() -> None:
        ...
        advance_selected_enroute_ship(False, "speed")
    ...
    def store_selected_ship_good() -> None:
        nonlocal port_action_destination, port_action_expires_at
        current = selected_ship
        if not can_take_action(active_player()):
            return
        if not current:
            return
    ...
    def advance_selected_enroute_ship(return_to_origin: bool = False, action_label: str | None = None) -> None:
        current = selected_ship
        if not can_take_action(active_player()):
            return
        ...
        if action_label is None:
            action_label = "return" if return_to_origin else "continue"
        old_pos = ship_screen_position(current)
        ...
        if not ship_is_enroute(current) and not pending_attack:
            ...
            current.attack_on_arrival = False
            complete_trade_if_ready(current, players, round_number, history_events, last_trade_summary)
        elif ship_is_enroute(current):
            record_sail_limit_action(current, action_label)
        mark_ship_done(current)
    ...
    while running:
        ...
        if news_popup_events and pygame.time.get_ticks() > news_popup_until:
            news_popup_events = []
            news_popup_until = 0
        if combat_popup_text and pygame.time.get_ticks() > combat_popup_until:
            combat_popup_text = ""
            combat_popup_until = 0
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if identity_selected_card is not None and identity_start_rect.collidepoint(event.pos):
                        selected_country = identity_card_countries[identity_selected_card]
                        selected_idx = EMPIRE_ORDER.index(selected_country)
                        active_player_idx = selected_idx
                        identity_screen_open = False
                        begin_active_player_turn()
                    elif identity_selected_card is None:
                        for idx, rect in enumerate(identity_card_rects):
                            if rect.collidepoint(event.pos):
                                identity_selected_card = idx
                                break
                continue
            elif game_over:
                ...
                if attack_button_rect.collidepoint(event.pos) and (
                    ...
                    resolve_pending_attack_once()
                elif attack_oil_rect.collidepoint(event.pos) and attack_result is None and can_take_action(player_by_country(players, pending_attack.attacker.owner)):
                    attacker_player = player_by_country(players, pending_attack.attacker.owner)
                ...
                if event.key == pygame.K_e and not (
                    market_open or trade_card_open or pirate_intel_open or treaty_open or transfer_open or history_open or build_open or new_ship_open or rules_open or overview_open
                ):
                ...
                if treaty_open:
                    ...
                    continue
                if market_open or trade_card_open or pirate_intel_open or history_open or build_open or new_ship_open or rules_open or overview_open:
                    continue
                if port_edit_mode:
                    continue
            ...
            elif overview_open and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if overview_close_rect.collidepoint(event.pos):
                    overview_open = False
            elif overview_open and event.type == pygame.MOUSEWHEEL:
                overview_scroll = max(0, min(overview_max_scroll, overview_scroll - event.y * 36))
            elif transfer_open and event.type == pygame.MOUSEWHEEL:
                if transfer_scroll_areas.get("from", pygame.Rect(0, 0, 0, 0)).collidepoint(mouse_pos):
                    transfer_from_scroll = max(
                        0,
                        min(transfer_max_scrolls.get("from", 0), transfer_from_scroll - event.y * 34),
                    )
                elif transfer_scroll_areas.get("to", pygame.Rect(0, 0, 0, 0)).collidepoint(mouse_pos):
                    transfer_to_scroll = max(
                        0,
                        min(transfer_max_scrolls.get("to", 0), transfer_to_scroll - event.y * 34),
                    )
                    ...
                    if not clicked_transfer:
                        for port_name, rect in transfer_to_port_rects:
                            if rect.collidepoint(event.pos):
                                ...
                                break
                    if not clicked_transfer:
                        for ship_id, rect in transfer_from_ship_rects:
                            if rect.collidepoint(event.pos):
                                if ship_id in transfer_from_ships:
                                    transfer_from_ships.remove(ship_id)
                                else:
                                    transfer_from_ships.add(ship_id)
                                clicked_transfer = True
                                break
                    if not clicked_transfer:
                        for ship_id, rect in transfer_to_ship_rects:
                            if rect.collidepoint(event.pos):
                                if ship_id in transfer_to_ships:
                                    transfer_to_ships.remove(ship_id)
                                else:
                                    transfer_to_ships.add(ship_id)
                                clicked_transfer = True
                                break
                    if not clicked_transfer:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and pirate_intel_close_rect.collidepoint(event.pos):
                    pirate_intel_open = False
                continue
            elif history_open:
                continue
                ...
                if new_ship_button_rect.collidepoint(event.pos):
                    open_new_ship_page()
                    clicked_tab = True
                if overview_button_rect.collidepoint(event.pos):
                    open_overview_page()
                    clicked_tab = True
        ...
        if identity_screen_open:
            identity_card_rects, identity_start_rect = draw_identity_page(
                ...
                font,
                small_font,
                identity_card_countries,
                identity_selected_card,
                mouse_pos,
            )
            pygame.display.flip()
            clock.tick(FPS)
        ...
        draw_news_port_highlights(screen, active_news)
        ...
        overview_button_rect = draw_overview_button(screen, font)
        ...
        for idx, rect in enumerate(player_tab_rects):
            if rect.collidepoint(mouse_pos):
                hovered_tab_player = players[idx]
                break
        if hovered_tab_player:
            draw_panel_popup(
                screen,
                small_font,
                f"{hovered_tab_player.country} Factories",
                player_factory_lines(hovered_tab_player),
                mouse_pos,
            )
        elif panel_hover_cards.get("resources", pygame.Rect(0, 0, 0, 0)).collidepoint(mouse_pos):
            ...
            lines = (
                [
                    "Pirate Intelligence",
                    f"Checks used: {pirate_intel_clicks}/5",
                    (
                        f"Next cost: ${PIRATE_INTEL_COSTS[pirate_intel_clicks]}"
                        if pirate_intel_clicks < len(PIRATE_INTEL_COSTS)
                        else "No checks left"
                    ),
                ]
        ...
        elif overview_button_rect.collidepoint(mouse_pos):
            draw_panel_popup(screen, small_font, "Overview", ["View all ports", "Ocean, type, resource, factory, owner, ships, entry"], mouse_pos)
        elif rules_button_rect.collidepoint(mouse_pos):
            draw_panel_popup(screen, small_font, "Rules", ["View complete game rules"], mouse_pos)
        draw_combat_result_banner(screen, font, combat_popup_text)
        if port_edit_mode:
        ...
        if market_open:
            ...
            overview_close_rect, overview_max_scroll = draw_overview_page(
                screen,
                title_font,
                font,
                small_font,
            ...
            pirate_intel_close_rect = draw_pirate_intel_page(
                screen,
                title_font,
                font,
                small_font,
            ...
            (
                ...
                transfer_from_ship_rects,
                transfer_to_ship_rects,
